...
if(isset($_GET['submit_login']) and !empty($_GET['input_user'])) {
	$pass_md5 = md5($_GET['input_pass']);
	if($pACL->authenticateUser($_GET['input_user'], $pass_md5)) {
		session_regenerate_id(TRUE);

		$_SESSION['elastix_user'] = $_GET['input_user'];
		$_SESSION['elastix_pass'] = $pass_md5;
		//header("Location: index.php");
		//writeLOG("audit.log", "LOGIN $_GET[input_user]: Web Interface login successful. Accepted password for $_GET[input_user] from $_SERVER[REMOTE_ADDR].");
		//exit;
	} else {
		$user = urlencode(substr($_GET['input_user'],0,20));
		if(!$pACL->getIdUser($_GET['input_user'])) // not exists user?
